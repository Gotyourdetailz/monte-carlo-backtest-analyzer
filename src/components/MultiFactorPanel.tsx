    ...
    <div className="glass-card panel-enter overflow-hidden">
      <div className="px-6 py-4 border-b border-[var(--border)]/50 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <span className={cn('badge', alphaSig ? 'badge-green' : 'badge-amber')}>
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="border border-[var(--border)]/60 rounded-lg overflow-hidden">
          <div className="grid grid-cols-12 text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-semibold border-b border-[var(--border)]/60 px-4 py-2">
          ...
          {report.factors.map((f) => {
            ...
            return (
              <div
                key={f.name}
                className="grid grid-cols-12 px-4 py-2 text-xs border-b border-[var(--border)]/30 last:border-b-0"
              >
                <div className="col-span-3 text-[var(--text-primary)]">{f.name}</div>
                <div className={cn('col-span-3 text-right metric-value', sig ? 'text-[var(--accent-blue)]' : 'text-[var(--text-primary)]')}>
                  {num(f.coefficient, 4)}
                </div>
                <div className="col-span-2 text-right metric-value text-[var(--text-secondary)]">{num(f.stdErr, 4)}</div>
                <div className={cn('col-span-2 text-right metric-value', Math.abs(f.tStat) >= 2 ? 'text-[var(--accent-blue)]' : 'text-[var(--text-primary)]')}>
                  {num(f.tStat, 2)}
                </div>
                <div className={cn('col-span-2 text-right metric-value', sig ? 'text-[var(--accent-green)]' : 'text-[var(--text-secondary)]')}>
                  {fmtP(f.pValue)}
                </div>
              </div>
            );
