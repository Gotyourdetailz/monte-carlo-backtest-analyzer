    ...
    <div className="glass-card animate-fade-in-up overflow-hidden">
      <div className="px-6 py-4 border-b border-[var(--border)]/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={cn('badge', Number(passRate) >= 80 ? 'badge-green' : Number(passRate) >= 50 ? 'badge-amber' : 'badge-red')}>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <div className={cn('text-lg metric-value animate-count-up', passColor)}>{passRate}%</div>
          </div>
        </div>
      </div>
            ...
            {scenarios.map((scenario, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-3">
                    <span className={cn('badge flex items-center gap-1 w-fit', style.badgeClass)}>
                      <Icon className="w-3 h-3" />
                      {scenario.severity}
                    </span>
                  </td>
                  <td className="px-6 py-3 metric-value text-right">
                    <span className={cn(scenario.capitalAfter < startingCapital ? 'text-[var(--accent-red)]' : 'text-[var(--accent-green)]')}>
                      ${Math.round(scenario.capitalAfter).toLocaleString()}
                    </span>
                  </td>
                  <td className="px-6 py-3 metric-value text-right">
                    <span className={cn(scenario.maxDrawdown > 0.3 ? 'text-[var(--accent-red)]' : scenario.maxDrawdown > 0.15 ? 'text-[var(--accent-amber)]' : 'text-[var(--text-primary)]')}>
                      {(scenario.maxDrawdown * 100).toFixed(1)}%
                    </span>
                  </td>
                  <td className="px-6 py-3 metric-value text-right">
                    <span className={cn(pnlImpact < 0 ? 'text-[var(--accent-red)]' : 'text-[var(--accent-green)]')}>
                      {pnlImpact >= 0 ? '+' : ''}${Math.round(pnlImpact).toLocaleString()}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-center">
                    <span className={cn('inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold', scenario.survives ? 'bg-[var(--accent-green)]/20 text-[var(--accent-green)]' : 'bg-[var(--accent-red)]/20 text-[var(--accent-red)]')}>
                      {scenario.survives ? '✓' : '✗'}
                    </span>
                  </td>
                </tr>
      ...
      {worstSurvivable && (
        <div className="px-6 py-4 border-t border-[var(--border)]/50 text-xs text-[var(--text-secondary)]">
